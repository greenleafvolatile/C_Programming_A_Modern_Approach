    ...
    while(grid[array][element]!='Z'){
        ...
        if(direction==DOWN){
            if(!directionsExplored[DOWN]){
                //If a move south was not explored, check if move south is within grid and would move to empty cell. 
                if(array < ARRAYS - 1 && grid[array+1][element]==EMPTY){
                    //move to adjacent cell to the south.
                    array++;
                }
                //If move is to cell outside of grid or to cell that is not empty, set explored to TRUE, check if there still
                //possible moves and, if so, generate next random move.
                else{
                    directionsExplored[DOWN]=!directionsExplored[DOWN];
                    checkForPossibleMoves(directionsExplored, grid);
                    continue;
                }
            }
...
void checkForPossibleMoves(bool directionsExplored[], char grid[][ELEMENTS]){
...
void printGrid(char grid[][ELEMENTS]){
